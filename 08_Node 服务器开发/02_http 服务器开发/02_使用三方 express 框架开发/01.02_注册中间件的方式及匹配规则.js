/*
  ...
  * app/router.get(路径, 中间件)、app.post(路径, 中间件)
    * 用来注册路径和方法匹配的中间件
    * 路径和方法匹配的中间件只能匹配到指定请求路径和请求方法的 http 请求
    * 比如有一个接口，我们只允许客户端通过 get 请求或 post 请求访问
  * app.use、app/router.get、app/router.post(路径, 中间件1, 中间件2, 中间件3, ...)
    * 用来一次性注册多个中间件
    * 主要用来把一个中间件拆分成多个中间件，每个中间件负责独立的业务逻辑，代码更加优雅
    * 比如一个登录接口，中间件1需要验证token，中间件2需要验证用户名和密码，中间件3需要从数据库读取用户信息，中间件4需要返回响应
  
  综上：
  * 1、express 里能注册三种类型的中间件：最普通的中间件、路径匹配中间件、路径和方法匹配中间件
  * 2、最普通的中间件用 app.use(中间件) 来注册
  * 3、路径匹配中间件用 app.use(路径, 中间件) 来注册
  * 4、路径和方法匹配中间件用 app/router.get(路径, 中间件)、app.post(路径, 中间件) 来注册
  * 5、且这几种注册方法都能一次性注册多个中间件
*/
