...
app.use(async (req, res, next) => {
  ...
  //  express 里 next() 函数的返回值是个 void，所以不可以用 await 来卡主
  next();
...
// 3、如果下一个中间件也遇到了 next() 函数，则继续跳转去匹配并执行下一个中间件、就像函数嵌套调用跳转那样，所以接下来打印了"中间件 3 before next"；但是因为当前中间件是个异步中间件，所以 next() 函数并不会等待下一个中间件的执行结果，而是会异步地继续执行当前中间件 next() 函数后的代码，所以接下来可能会打印"中间件 2 after next"、此时中间件 2 的生命周期就算走完了
// 4、如果遇到 next() 函数后没有下一个中间件了或者给客户端返回响应了，那就执行完当前中间件，并不会像函数嵌套调用释放调用栈那样一层一层释放，而是直接执行当前中间件 next() 函数后的代码