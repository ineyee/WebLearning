...
const registerRepository = require("../../repository/user/register.repository");
// 导入注册模块响应错误的状态码和消息
const registerError =
  require("../../config/response/response-error.config").REGISTER;
// 导入加密工具函数
...
class RegisterService {
  ...
  async register(params) {
    ...
    if (readUserResult.length > 0) {
      throw {
        code: registerError.EMAIL_IS_ALREADY_EXISTS.code,
        message: registerError.EMAIL_IS_ALREADY_EXISTS.message,
      };
    }

    // 2、做业务
    // 调用数据层的 API，将客户端传递过来的参数存储到数据库中
    params.password = md5Password(params.password); // 这里可以加密密码
    const result = await registerRepository.createUser(params);
    return result;
  }
}
