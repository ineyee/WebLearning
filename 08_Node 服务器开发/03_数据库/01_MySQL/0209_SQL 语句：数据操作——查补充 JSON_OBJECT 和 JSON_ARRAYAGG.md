...
比如我们设计了如下歌手表（主表1）、歌曲表（主表2）和中间表（外键都是在中间表里，中间表包含两个外键，分别指向主表1和主表2的主键）:
...
而要想实现这样的查询结果，就不能用默认的查询语句了，得用一下 `JSON_ARRAYAGG 这个聚合函数——用来把查询到的数据搞成一个数组`，而 `JSON_ARRAYAGG 必须配合 GROUP BY 先把数据分组后才能把各组的数据搞成分组`：
