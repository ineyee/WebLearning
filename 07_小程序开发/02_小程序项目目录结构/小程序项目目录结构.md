...
* ✅ index.json：当前界面的配置文件
	如果我们小程序的某个界面有自己独特的设计，不想共用app.json里window的全局配置，那可以在自己的json文件里进行单独配置，单独配置会覆盖app.json里window中同名的配置项，属性跟app.json里window的属性差不多
* ✅ index.wxml：当前界面的结构文件
	可以使用小程序内置的组件，也可以使用我们自定义的组件（注意自定义的组件必须在当前界面的配置文件-usingComponents里注册后才能使用）
