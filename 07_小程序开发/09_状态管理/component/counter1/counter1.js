  ...
  */
  ...
  methods: {
    /*
      ...
      所以组件里还是需要定义跟 store 里同名的方法的，来调用 store 里的方法修改共享的数据，因为我们无法在这里直接修改 store 里共享的数据

      可见小程序里的 store 真就是个数据和方法的中转站，纯粹是为了能够共享数据，如果数据和方法是独享而不需要共享的话，那直接写在组件里要比使用状态管理简单多了，导致复杂的主要原因就是 wxml 只能直接使用 .js 里文件里的东西
    */
    add() {
      useCounterStore.dispatch("add");
    },
    ...
    */
    counterCallback(value) {
      console.log("couter1 监听 counter:", value)
