...
// package.json文件就会创建出来了（注意：里面指定了项目的默认入口文件index.js————./src/index.js，所以“your_project”项目的入口文件必须得是这个名字，否则webpack打包是找不到入口文件的，除非我们手动修改这个默认值，这里先不修改）
...
// webpack就会帮我们打包项目了，打包成功后，webpack会在项目的根目录下默认创建一个名为“dist”的文件夹（distribution发布的缩写）和一个名为main.js的文件（当然我们也可以修改产物的输出目录和输出文件名，这里先不修改），打包的产物就都放在这个目录下了，我们将来部署到服务器上的东西就是它们了
// 
